...
// email además de por WhatsApp. Solo necesita dos variables de entorno:
//   RESEND_API_KEY  -> la clave de la cuenta (empieza por "re_")
//   RESEND_FROM     -> remitente verificado, p. ej. "Dental Fortes <hola@tudominio.com>"
...
function buildHtml({ text, headerText, footerText, title }) {
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:14px;padding:28px 30px;box-shadow:0 6px 24px -12px rgba(30,50,90,.25)">
    ...
    <div style="margin-top:18px;font-size:11.5px;color:#9aa4b2">
      ...
      Si no desea recibir más comunicaciones comerciales, responda a este correo indicando "BAJA".
    </div>
  </div>
</body></html>`;
}
...
async function sendEmail({ to, subject, text, headerText, footerText, replyTo }) {
  if (!isConfigured()) throw new Error("Resend no está configurado (faltan RESEND_API_KEY o RESEND_FROM).");
...
module.exports = { isConfigured, sendEmail, resendFrom, buildHtml };
