...
async function resolveTreatment({ especialidad, motivo }) {
  const { data: treatments } = await supabase
    .from("df_treatments").select("id, name, duration_minutes, is_first_visit").eq("active", true);
  const list = treatments || [];
  if (!list.length) return { id: null, name: null, duration_minutes: 30, is_first_visit: true };
