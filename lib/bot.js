...
const { resolveProfessional, busyProfessionals, assignCabinet, localWeekdayAndTime, addMinutes, matchesSpecialty, isGeneralist, isOnVacation, isClinicClosed, continuityProfessional, availableAt, withinBookingWindow, MAX_CABINETS, CAPACITY_REASONS } = require("./scheduling");
const { ensurePaymentForAppointment, clearPendingPaymentForAppointment } = require("./billing");
// Las dos confirmaciones de una cita: la de recepción y la de asistencia del paciente.
const { updateAppointment, marcarAsistencia, RESET_AL_MOVER } = require("./citas");
...
// Avisos internos que el CRM escribe en la conversación cuando el asistente falla. Son
// para recepción, no forman parte de lo que se habló con el paciente: si se los pasamos
// al modelo como palabras suyas, arrastra el error al turno siguiente.
const INTERNAL_MARK = "⚠️ El asistente no pudo responder";

async function loadHistory(conversationId, limit = 24, currentUserText = "") {
  const { data } = await supabase
    .from("df_messages")
    .select("role, content")
    .eq("conversation_id", conversationId)
    ...
    .limit(limit);
  // Vuelve a orden cronológico y descarta contenidos vacíos y avisos internos.
  const rows = (data || [])
    .filter((m) => m.content && String(m.content).trim())
    .filter((m) => !String(m.content).startsWith(INTERNAL_MARK))
    .reverse();
  ...
  // La conversación tiene que TERMINAR en el paciente: los modelos actuales rechazan que
  // el último mensaje sea del asistente ("does not support assistant message prefill",
  // error 400) y el bot se quedaba mudo. Pasa cuando el paciente manda dos mensajes
  // seguidos: mientras se prepara la respuesta del primero, el segundo ya se ha guardado,
  // y la respuesta al primero se graba entremedias, dejando el historial en 'assistant'.
  // Se vuelve a poner al final lo que acaba de escribir, que es justo lo que hay que
  // contestar; así el modelo ve que del mensaje anterior ya se encargó.
  const ultimo = msgs[msgs.length - 1];
  if (ultimo && ultimo.role !== "user") {
    const pendiente = String(currentUserText || "").trim();
    if (pendiente) msgs.push({ role: "user", content: pendiente });
    else msgs.pop(); // sin nada que contestar, se descarta la cola del asistente
  }
  // Si al quitar la cola nos quedamos sin conversación, no hay nada que enviar.
...
async function upcomingAppointments(patientIds, extraSelect = "") {
  ...
  const { data } = await supabase
    ...
    // "*" (y no columna a columna) para que patient_confirmed_at llegue si la migración
    // sql/confirmacion-asistencia.sql está hecha y no reviente si todavía no lo está.
    .select("*" + (extraSelect ? ", " + extraSelect : ""))
  ...
  return [
    ...
    },
    {
      ...
      run: async () => {
        ...
        // Lo que se anota entre paréntesis es si el PACIENTE ya ha dicho que va a venir,
        // que es lo único que le concierne a él. Que recepción haya validado la cita es
        // un trámite interno: para el paciente la cita está cogida, y decirle que está
        // "pendiente de confirmar" solo le haría pensar que tiene que hacer algo.
        const fmt = (a) => `- ${describeAppt(a)} (${a.patient_confirmed_at ? "ya ha confirmado que viene" : "aún no ha confirmado su asistencia"})`;
        return "Citas PROGRAMADAS AHORA MISMO (estado REAL de la agenda; esto manda sobre cualquier cosa dicha antes en el chat). Entre paréntesis, a nombre de quién está cada cita (puede ser un hijo/a) y si ya confirmó que acudirá:\n" +
          appts.map(fmt).join("\n");
      },
    },
    {
      ...
      run: async (input) => {
        ...
        const r = await resolveProfessional({
          ...
          busyProfessionalIds: await busyProfessionals(supabase, start.toISOString(), end.toISOString()),
        });
        if (!r.professional) {
      ...
      run: async (input) => {
        ...
        if (wt) {
          const r = await resolveProfessional({
            ...
            endHhmm: addMinutes(wt.hhmm, dur), especialidad: input.especialidad,
            allowedProfessionalIds: t.professional_ids, dateStr: String(input.fecha_hora_inicio).slice(0, 10),
            // Sin esto se podía elegir a quien ya tenía cita a esa hora y perder el
            // hueco, teniendo un compañero libre que sí podía atenderla.
            busyProfessionalIds: await busyProfessionals(supabase, start.toISOString(), end.toISOString()),
          });
    ...
    },
    {
      definition: {
        ...
        description:
          "Úsala cuando el paciente quiera CANCELAR (anular) una cita que ya tiene, NO cuando quiera cambiarla de día/hora (para eso está reagendar_cita). Va en DOS pasos: la primera vez te dirá que le ofrezcas otro hueco antes de anular; si aun así insiste en anularla, vuelve a llamarla con insiste=true y la cita se anula de verdad.",
        input_schema: {
          type: "object",
          properties: {
            ...
            insiste: { type: "boolean", description: "true SOLO si ya le has ofrecido cambiarla de día/hora y ha dicho claramente que quiere anularla igualmente. Con true la cita se anula de verdad." },
          },
        },
      },
      run: async (input) => {
        ...
        // PRIMER PASO: antes de perder la cita, se le ofrece cambiarla. Muchas
        // cancelaciones son en realidad "no me viene bien ese día", y si se anula sin
        // preguntar el paciente se queda sin tratamiento y la clínica sin el hueco.
        if (!input.insiste) {
          return `El paciente quiere anular su cita del ${cuando}. ANTES de anularla, ofrécele cambiarla ` +
            "de día u hora: usa proponer_hueco con el mismo motivo de la cita y enséñale huecos REALES " +
            "(no propongas días ni horas por tu cuenta). Dilo en una línea, sin insistir ni presionarle. " +
            "· Si acepta uno de los huecos, muévela con reagendar_cita. " +
            "· Si te dice que la quiere anular igualmente, vuelve a llamar a solicitar_cancelacion con " +
            "insiste=true y la misma fecha_hora_cita. NO le digas todavía que la cita está anulada.";
        }

        // SEGUNDO PASO: ha dicho que la quiere anular igual. Se anula de verdad y
        // desaparece de la agenda (el panel no pinta las canceladas).
        const { error: errCancel } = await supabase.from("df_appointments")
          .update({ status: "cancelled" }).eq("id", appt.id);
        if (errCancel) return "No se ha podido anular la cita: " + errCancel.message;

        // El cobro pendiente de una visita que ya no se hace deja de contar como
        // facturación (igual que al cancelarla desde el panel).
        await clearPendingPaymentForAppointment(supabase, appt.id).catch(() => {});

        // Queda constancia para recepción: el aviso apunta a la cita CONCRETA
        // (appointment_id) y nace ya resuelto ('handled'), porque la cita está anulada y
        // no hay nada que gestionar; sirve de registro de por qué desapareció del hueco.
        // (el constructor de consultas de Supabase es un "thenable", no una promesa:
        //  no tiene .catch(), así que el error se mira en la respuesta)
        const { error: errAviso } = await supabase.from("df_cancellation_requests").insert({
          ...
          reason: [reason, "Anulada por el paciente desde WhatsApp"].filter(Boolean).join(" · "),
          status: "handled",
          handled_at: new Date().toISOString(),
        });
        // La cita ya está anulada, que es lo que importa: si el registro para recepción
        // falla, se deja constancia en el log y se sigue.
        if (errAviso) console.error("[cancelacion/aviso]", errAviso.message);

        return `Cita ANULADA (${cuando}): ya no aparece en la agenda y recepción tiene el aviso. ` +
          "Confírmaselo al paciente en una línea, con amabilidad, y ofrécele volver a pedir cita cuando le venga bien. " +
          "No le pidas que confirme nada más ni vuelvas a llamar a esta herramienta para esta cita.";
      },
    },
    {
      definition: {
      ...
      run: async (input) => {
        ...
        // Lo que confirma el paciente es que VA A VENIR. Que la cita esté validada por
        // recepción es otra cosa y no la decide él (ver lib/citas.js).
        const { yaEstaba, error } = await marcarAsistencia(supabase, appt);
        if (error) return "No se ha podido confirmar la asistencia: " + error.message;
        if (yaEstaba) {
          return `Ya constaba que va a acudir a esa cita (${describeAppt(appt)}). Díselo con naturalidad para que se quede tranquilo; no hace falta nada más.`;
        }
        return `Asistencia CONFIRMADA (${describeAppt(appt)}): queda anotado que va a venir y dejan de enviársele recordatorios de esa cita. Agradéceselo en una línea y no le pidas que la confirme otra vez.`;
      },
    },
    {
      definition: {
      ...
      run: async (input) => {
        ...
        if (!chosenPro) {
          ...
          const r = await resolveProfessional({
            ...
            // Se excluye la propia cita: al moverla, su hueco de origen queda libre.
            busyProfessionalIds: await busyProfessionals(supabase, start.toISOString(), end.toISOString(), appt.id),
          });
          if (!r.professional) {
            if (r.reason === "especialista_no_disponible" && r.preferred) {
        ...
        // Al moverla vuelve a estar pendiente del repaso de recepción, y lo que el
        // paciente hubiera confirmado era el hueco viejo: se limpia junto con los
        // recordatorios ya enviados (RESET_AL_MOVER, en lib/citas.js).
...
async function handleMessage({ channel, phone, token, name, email, text, imageUrl }) {
  ...
  // Se le pasa lo que ACABA de escribir el paciente: si entre medias se coló una
  // respuesta del asistente (dos mensajes seguidos), el historial se cierra con ella.
  const history = await loadHistory(conversation.id, 24, text || (imageUrl ? "📷 Imagen" : ""));
