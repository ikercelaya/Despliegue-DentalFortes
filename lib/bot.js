...
// Marca que se escribe en la conversación cuando se le envía una CAMPAÑA al paciente.
// Corta el contexto: el bot no debe retomar lo que se hablara antes de ese aviso
// (p. ej. seguir ofreciendo una hora de una conversación de hace semanas).
...
async function loadHistory(conversationId, limit = 24) {
  ...
  const rows = (data || [])
    ...
    .reverse();
  // Si hay una marca de campaña, el historial empieza DESPUÉS de ella.
  let cut = -1;
  rows.forEach((m, i) => { if (String(m.content).startsWith(CAMPAIGN_CONTEXT_MARK)) cut = i; });
...
// ---------------- Herramientas ----------------
// `flags` permite a las herramientas avisar a handleMessage de acciones que hay que
// hacer al terminar el turno (p. ej. enviar la plantilla de consentimiento).
function buildTools(conversation, flags = {}) {
  ...
  return [
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        try {
          const { data: pt } = await supabase
            .from("df_patients").select("marketing_consent, tags").eq("id", patientId).maybeSingle();
          // A quien YA respondió que no (etiqueta "sin_marketing") no se le vuelve a preguntar.
          const yaDijoQueNo = pt && (pt.tags || []).includes("sin_marketing");
          if (pt && pt.marketing_consent !== true && !yaDijoQueNo) flags.sendConsentTemplate = true;
        } catch (_e) {}
        return (
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        // Guardar el correo es el último paso del alta: aprovechamos para pedirle el
        // consentimiento de marketing si aún no lo ha decidido.
        try {
          if (patientId) {
            const { data: pt } = await supabase
              .from("df_patients").select("marketing_consent").eq("id", patientId).maybeSingle();
            if (pt && pt.marketing_consent !== true) flags.sendConsentTemplate = true;
          }
        } catch (_e) {}
...
async function handleMessage({ channel, phone, token, name, email, text, imageUrl }) {
  ...
  const flags = {};
  const tools = buildTools(conversation, flags);
  ...
  // sendConsentTemplate: el webhook enviará después la plantilla de consentimiento
  // de marketing (con sus botones) si en este turno se ha cerrado el alta del paciente.
  return { reply: finalReply, conversation, language, sendConsentTemplate: !!flags.sendConsentTemplate };
}

module.exports = { handleMessage, getOrCreateConversation, saveMessage, phoneVariants, CAMPAIGN_CONTEXT_MARK };
