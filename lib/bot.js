...
const { resolveProfessional, assignCabinet, localWeekdayAndTime, addMinutes, matchesSpecialty, isGeneralist, isOnVacation, isClinicClosed, continuityProfessional, withinBookingWindow, MAX_CABINETS, CAPACITY_REASONS } = require("./scheduling");
...
async function findOrCreatePatient({ phone, name, conversationId }) {
  ...
  //    para no crear un paciente duplicado.
  if (cleanPhone) {
    const { data: existing } = await supabase
      .from("df_patients").select("id").in("phone", phoneVariants(cleanPhone)).limit(1).maybeSingle();
  ...
  return [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const { data: appts } = await supabase
          .from("df_appointments")
          .select("starts_at, status, df_treatments(name), df_professionals(name)")
        ...
        const fmt = (a) =>
          `- ${new Date(a.starts_at).toLocaleString("es-ES", { weekday: "long", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit", timeZone: "Europe/Madrid" })}` +
          `${a.df_treatments?.name ? " · " + a.df_treatments.name : ""}` +
          ` (${a.status === "confirmed" ? "confirmada" : "pendiente de confirmar"})`;
        return "Citas PROGRAMADAS del paciente AHORA MISMO (estado REAL de la agenda; esto manda sobre cualquier cosa dicha antes en el chat):\n" + appts.map(fmt).join("\n");
      },
    },
    {
      definition: {
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        const horizon = new Date(from.getTime() + 22 * 86400000);
        const { data: appts } = await supabase
        ...
        for (let d = 0; d <= 21 && slots.length < 3; d++) {
          ...
          if (await isClinicClosed(supabase, dayStr)) continue;   // clínica CERRADA (vacaciones): ni ofrecer
          let best = null;
          for (const p of cand) {
          ...
          const wtb = localWeekdayAndTime(best.iso);
          const rb = await resolveProfessional({
            ...
            allowedProfessionalIds: t.professional_ids, dateStr: best.iso.slice(0, 10),
          });
          if (!rb.professional) continue;
          const capb = await assignCabinet({
            supabase, startISO: new Date(best.ms).toISOString(),
            endISO: new Date(best.ms + dur * 60000).toISOString(),
            isFirstVisit: isFV, professionalId: rb.professional.id,
          });
          if (!capb.ok) continue;
          best.prof = rb.professional.name;   // el profesional que asignará la reserva
          slots.push(best);
        }
        if (!slots.length) {
          return "No hay huecos libres en los próximos 21 días desde esa fecha para ese tipo de visita (contando horarios, vacaciones y cierres de la clínica). Díselo con naturalidad y, si quiere, vuelve a llamar a proponer_hueco con un desde_fecha posterior. NO ofrezcas fechas por tu cuenta.";
        }
        return "HUECOS LIBRES REALES para la cita (ya descontadas vacaciones, bloqueos y cierres), ORDENADOS del MÁS TEMPRANO al más tardío. Ofrécele SIEMPRE el PRIMERO (el más pronto) y, como alternativa, el segundo. Cuando acepte, agéndalo con crear_cita usando exactamente ese fecha_hora_inicio y el mismo motivo:\n" +
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        const patientId = await findOrCreatePatient({ phone: input.telefono || channelPhone, name: input.nombre, conversationId: conversation.id });
    ...
    },
    {
      definition: {
        ...
        description:
          "Úsala cuando el paciente quiera CANCELAR (anular) una cita que ya tiene, NO cuando quiera cambiarla de día/hora. Avisa a recepción para que le contacten; tú NO canceles la cita.",
        input_schema: {
          type: "object",
          properties: { motivo: { type: "string", description: "Motivo de la cancelación, si lo indica (opcional)" } },
        },
      },
      run: async (input) => {
        const { data: conv } = await supabase
        ...
        if (conv?.patient_id) {
          const { data } = await supabase
            .from("df_appointments").select("id, starts_at")
            .eq("patient_id", conv.patient_id).in("status", ["pending", "confirmed"])
        ...
        const { error } = await supabase.from("df_cancellation_requests").insert({
          ...
          customer_name: conv?.customer_name || conversation.customer_name || null,
          customer_phone: conv?.customer_phone || conversation.customer_phone || null,
          reason: String(input.motivo || "").trim() || null,
          status: "pending",
        });
        if (error) return "No se ha podido registrar la solicitud de cancelación: " + error.message;
        return "Solicitud de cancelación registrada para recepción. Dile al paciente con empatía que has trasladado su solicitud y que recepción se pondrá en contacto con él en breve para gestionarla. NO le confirmes que la cita ya está cancelada.";
      },
    },
    {
      definition: {
        ...
        input_schema: {
          type: "object",
          properties: { fecha_hora_inicio: { type: "string", description: "Nuevo inicio en ISO local (Madrid), p. ej. 2026-07-13T10:00" } },
          required: ["fecha_hora_inicio"],
        },
      },
      run: async (input) => {
        ...
        const { data: conv } = await supabase
          .from("df_conversations").select("patient_id").eq("id", conversation.id).maybeSingle();
        if (!conv?.patient_id) return "No localizo al paciente para reagendar. Pídele su nombre completo y búscalo con buscar_paciente.";
        const { data: appt } = await supabase
          .from("df_appointments").select("*, df_treatments(duration_minutes, is_first_visit)")
          .eq("patient_id", conv.patient_id).in("status", ["pending", "confirmed"])
          .gte("starts_at", new Date().toISOString()).order("starts_at", { ascending: true }).limit(1).maybeSingle();
        if (!appt) return "El paciente no tiene ninguna cita futura que reagendar. Si quiere una cita nueva, agéndala con crear_cita.";
        ...
        const end = new Date(start.getTime() + dur * 60000);
        const r = await resolveProfessional({
          supabase, patientId: conv.patient_id, weekday: wt.weekday, hhmm: wt.hhmm,
          endHhmm: addMinutes(wt.hhmm, dur), dateStr: String(input.fecha_hora_inicio).slice(0, 10),
        });
        if (!r.professional) {
          if (r.reason === "clinic_closed") {
          ...
          if (r.reason === "especialista_no_disponible" && r.preferred) {
            return `Su profesional (${r.preferred.name}) no tiene consulta (o está de vacaciones) ese día/hora. Propón otro momento dentro de su horario con proponer_hueco.`;
          }
          return "No hay hueco a esa hora para su cita. Ofrécele otro con proponer_hueco.";
        }
        const cap = await assignCabinet({
          supabase, startISO: start.toISOString(), endISO: end.toISOString(),
          isFirstVisit: isFV, professionalId: r.professional.id, excludeId: appt.id,
        });
        if (!cap.ok) return `Ese hueco no está libre (${CAPACITY_REASONS[cap.reason] || "no disponible"}). Ofrécele otra hora con proponer_hueco.`;
        const { error } = await supabase.from("df_appointments").update({
          starts_at: start.toISOString(), ends_at: end.toISOString(),
          professional_id: r.professional.id, cabinet: cap.cabinet,
        ...
        return "Cita reagendada correctamente al nuevo día y hora (queda pendiente de confirmar por recepción). Confírmaselo al paciente con naturalidad; no vuelvas a crear otra cita.";
      },
    },
    {
      definition: {
