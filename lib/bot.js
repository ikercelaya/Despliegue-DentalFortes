...
// Día de la semana (0=lunes..6=domingo, como en df_professional_schedules) y hora HH:MM
// a partir de una cadena ISO local "YYYY-MM-DDTHH:MM" (hora de la clínica).
...
// Elige un profesional ACTIVO cuyo horario cubra ese día y hora. Si se indica una
// especialidad/cargo, prioriza al que coincida; si no, el primero disponible.
async function pickProfessional({ weekday, hhmm, especialidad }) {
  const { data: pros } = await supabase
    .from("df_professionals")
    .select("id, name, specialty, active, df_professional_schedules(weekday, start_time, end_time)")
    .eq("active", true);
  const disponibles = (pros || []).filter((p) =>
    (p.df_professional_schedules || []).some((s) =>
      Number(s.weekday) === weekday &&
      String(s.start_time).slice(0, 5) <= hhmm &&
  ...
  if (especialidad) {
    ...
    const match = disponibles.find((p) => (p.specialty || "").toLowerCase().includes(needle));
  ...
  return [
    ...
    },
    {
      definition: {
        name: "crear_cita",
        description:
          "Registra una primera visita PENDIENTE de confirmar por recepción. Úsala solo cuando el paciente haya confirmado día y hora concretos y tengas su nombre y teléfono. Llama a esta herramienta UNA sola vez por cita: si ya la has creado, NO vuelvas a llamarla.",
        input_schema: {
          type: "object",
          properties: {
      ...
      run: async (input) => {
        ...
        // Profesional según horario (día/hora) y, si se indica, especialidad/cargo.
        let professionalId = null;
        let profNombre = null;
        const wt = localWeekdayAndTime(input.fecha_hora_inicio);
        if (wt) {
          const pro = await pickProfessional({ weekday: wt.weekday, hhmm: wt.hhmm, especialidad: input.especialidad });
          if (pro) { professionalId = pro.id; profNombre = pro.name; }
        }
