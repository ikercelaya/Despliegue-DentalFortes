  ...
  return [
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        // Evita el AUTO-CHOQUE: si el propio paciente (o un hijo suyo) ya tiene una cita
        // que pisa esa franja, NO es un "hueco ocupado" cualquiera: o es la misma cita que
        // acaba de reservar, o es la cita que quiere MOVER a esa hora.
        const { ids: ownIds } = await relatedPatientIds(conversation.id);
        if (ownIds.length) {
