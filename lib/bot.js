...
async function loadHistory(conversationId, limit = 20) {
  // Cargamos los ÚLTIMOS mensajes (los más recientes) para que el bot vea
  // siempre el mensaje actual del paciente, y luego los devolvemos en orden
  // cronológico (antiguo -> nuevo).
  const { data } = await supabase
    .from("df_messages")
    .select("role, content")
  ...
  return [
    {
      ...
      run: async (input) => {
        ...
        if (!matches || !matches.length) {
          ...
          return `No consta ningún paciente con el nombre "${q}". Trátalo como PACIENTE NUEVO: pídele los datos que falten (teléfono y motivo). IMPORTANTE: el correo NO se lo pidas ahora; se lo pedirás DESPUÉS de agendar la cita.`;
        }
        const top = matches[0];
        ...
        return (
          ...
          `El correo que consta es "${top.email || "ninguno"}": DESPUÉS de agendar la cita, pregúntale si ese correo es correcto.`
        );
      },
    },
    {
      ...
      run: async (input) => {
        ...
        return (
          "Cita registrada como PENDIENTE de confirmación. Recepción la confirmará. Comunícaselo al paciente. " +
          "AHORA gestiona el CORREO: si es paciente NUEVO, pídeselo para registrarlo; si YA era paciente, " +
          "pregúntale si el correo que consta en su ficha es correcto (sí/no)."
        );
      },
    },
    {
