...
const { resolveProfessional, assignCabinet, localWeekdayAndTime, addMinutes, matchesSpecialty, isGeneralist, isOnVacation, withinBookingWindow, MAX_CABINETS, CAPACITY_REASONS } = require("./scheduling");
      ...
      run: async (input) => {
        ...
        if (!r.professional) {
          if (r.reason === "clinic_closed") {
            return "NO disponible: la clínica está CERRADA (vacaciones) ese día. Explícaselo y propón una fecha posterior a la reapertura.";
          }
          if (r.reason === "especialista_no_disponible" && r.preferred) {
            return `NO disponible: para ese caso debe atender ${r.preferred.name} (${r.preferred.specialty}), que no tiene consulta (o está de vacaciones) ese día/hora. Propón otro momento dentro de su horario.`;
          }
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        for (let d = 0; d <= 21 && slots.length < 3; d++) {
          ...
          let best = null;
          for (const p of cand) {
            if (isOnVacation(p, dayStr)) continue;   // profesional de vacaciones ese día
