...
const { resolveProfessional, assignCabinet, localWeekdayAndTime, addMinutes, matchesSpecialty, isGeneralist, CAPACITY_REASONS } = require("./scheduling");
  ...
  return [
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        // Profesionales que PUEDEN atender ese tipo de primera visita:
        //  - especialidad concreta (ortodoncia, endodoncia, cirugía...) -> quien la ofrece.
        //  - general / limpieza / higiene / revisión / sin especialidad -> los GENERALISTAS
        //    (Irene y Mishelle), aunque la especialidad registrada de una no sea "general".
        ...
        // Primera visita = INDIVIDUAL: cualquier cita que solape ese tramo bloquea el hueco.
        const now = new Date();
          ...
          for (const p of cand) {
            for (const s of (p.df_professional_schedules || [])) {
              ...
              for (let m = st; m + dur <= en; m += dur) {
                ...
                if (overlaps(startMs, startMs + dur * 60000)) continue;
        ...
        if (!slots.length) {
          return "No he encontrado huecos libres en los próximos 21 días para ese tipo de primera visita. Pídele su preferencia de día/hora y compruébala con comprobar_disponibilidad.";
        }
        return "HUECOS LIBRES para primera visita, ORDENADOS del MÁS TEMPRANO al más tardío. Ofrécele SIEMPRE el PRIMERO (el más pronto) y, como alternativa, el segundo. Cuando acepte, agéndalo con crear_cita usando exactamente ese fecha_hora_inicio:\n" +
    ...
    },
    {
      ...
      run: async (input) => {
        ...
        // Nunca agendes una primera visita sin un profesional que la atienda: si a esa
        // hora no hay generalista/especialista adecuado, propón otra hora (usa proponer_hueco).
        if (!professionalId) {
          return "A esa hora no hay ningún profesional que atienda ese tipo de primera visita. " +
