...
async function resolveProfessional({ supabase, patientId, weekday, hhmm, endHhmm, especialidad }) {
  ...
  // 3) Por defecto, un generalista disponible (revisión / primera visita).
  const g = avail.find((p) => isGeneralist(p));
  if (g) return { professional: g, reason: "generalista" };

  // 4) Cualquier profesional disponible a esa hora.
  if (avail.length) return { professional: avail[0], reason: "disponible" };
  return { professional: null, reason: "nadie_disponible" };
}
...
async function assignCabinet({ supabase, startISO, endISO, isFirstVisit, professionalId, excludeId, desiredCabinet, maxCabinets = MAX_CABINETS }) {
  ...
  // Mismo profesional ya ocupado en ese tramo.
  if (professionalId && rows.some((r) => r.professional_id === professionalId)) {
    return { ok: false, reason: "profesional_ocupado" };
  }

  if (isFirstVisit) {
    if (rows.length > 0) return { ok: false, reason: "primera_visita_no_individual" };
    return { ok: true, cabinet: 1 };
  }

  // Citas normales: no pueden solaparse con una primera visita.
  if (rows.some((r) => r.is_first_visit)) return { ok: false, reason: "solapa_primera_visita" };
