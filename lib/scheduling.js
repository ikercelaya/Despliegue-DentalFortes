...
// Última hora a la que se puede EMPEZAR una cita (la clínica cierra poco después):
// 19:00 de lunes a jueves y 13:45 el viernes. Sábado y domingo no se reserva.
// weekday: 0=lunes .. 6=domingo.
const LAST_BOOKING_TIME = { 0: "19:00", 1: "19:00", 2: "19:00", 3: "19:00", 4: "13:45" };
function withinBookingWindow(weekday, hhmm) {
  const limit = LAST_BOOKING_TIME[weekday];
  if (!limit) return false;               // sábado/domingo: no se agenda
  return String(hhmm).slice(0, 5) <= limit;
}
...
// ¿El profesional está de VACACIONES/ausente ese día? dateStr = "YYYY-MM-DD" (fecha local).
// Los rangos [start_date, end_date] incluyen ambos extremos.
function isOnVacation(p, dateStr) {
  if (!dateStr) return false;
  const d = String(dateStr).slice(0, 10);
  return (p.df_professional_time_off || []).some((v) => {
    const s = String(v.start_date).slice(0, 10);
    const e = String(v.end_date || v.start_date).slice(0, 10);
    return s <= d && d <= e;
  });
}

// ¿La clínica está CERRADA (vacaciones generales) ese día? dateStr = "YYYY-MM-DD".
async function isClinicClosed(supabase, dateStr) {
  if (!dateStr) return false;
  const d = String(dateStr).slice(0, 10);
  try {
    const { data } = await supabase
      .from("df_clinic_closures").select("id").lte("start_date", d).gte("end_date", d).limit(1);
...
// Profesionales que YA tienen una cita que pisa ese tramo. Hace falta para no elegir a
// alguien que trabaja ese día pero está ocupado justo a esa hora: antes se le asignaba
// igual y la reserva se caía después en assignCabinet ("profesional ocupado"), aunque
// hubiera otro compañero libre que sí podía atenderla.
async function busyProfessionals(supabase, startISO, endISO, excludeId) {
  ...
  return new Set((data || [])
    .filter((r) => r.id !== excludeId && r.professional_id)
...
async function resolveProfessional({ supabase, patientId, weekday, hhmm, endHhmm, especialidad, allowedProfessionalIds, dateStr, busyProfessionalIds }) {
  // Si la clínica está de vacaciones generales ese día, no hay citas para nadie.
  if (dateStr && (await isClinicClosed(supabase, dateStr))) {
    return { professional: null, reason: "clinic_closed" };
  }
  ...
  // e isGeneralist() cae a la heurística por especialidad. Incluimos las vacaciones
  // (df_professional_time_off) para no ofrecer a un profesional ausente ese día.
  ...
  // "Disponible" = trabaja esa franja, no está de vacaciones ese día Y no tiene ya otra
  // cita a esa hora. Lo último es lo que permite repartir: si Mishelle está ocupada a las
  // 12:00 pero Irene está libre, la limpieza se le da a Irene en vez de rechazar el hueco.
  const ocupados = busyProfessionalIds instanceof Set
    ? busyProfessionalIds
    : new Set(busyProfessionalIds || []);
  const avAt = (p) => availableAt(p, weekday, hhmm, endHhmm) && !isOnVacation(p, dateStr) && !ocupados.has(p.id);
  const avail = all.filter(avAt);

  // 1) Continuidad paciente-doctor.
  const cont = await continuityProfessional(supabase, patientId, byId);
  if (cont) {
    if (avAt(cont)) return { professional: cont, reason: "continuidad" };
  ...
  // 2) Tratamiento con profesionales FIJADOS en el CRM (apartado Tratamientos):
  //    puede atenderlo CUALQUIERA de ellos que tenga consulta a esa hora (p. ej.
  //    prótesis la cubren Irene y Ana: vale cualquier día en que trabaje una de
  //    las dos). Preferimos quien case con la especialidad pedida, luego un
  //    generalista y, si no, el resto de asignados.
  if (Array.isArray(allowedProfessionalIds) && allowedProfessionalIds.length) {
    const pick = (especialidad && avail.find((p) => matchesSpecialty(p, especialidad)))
      || avail.find((p) => isGeneralist(p))
      || avail[0];
    if (pick) return { professional: pick, reason: "tratamiento_asignado" };
    const specAny = especialidad ? all.find((p) => matchesSpecialty(p, especialidad)) : null;
    if (specAny && !isGeneralist(specAny)) {
      return { professional: null, reason: "especialista_no_disponible", preferred: specAny };
    }
    return { professional: null, reason: "sin_profesional_disponible" };
  }

  // 3) Paciente nuevo / sin historial: enrutar por especialidad si se indica.
  if (especialidad) {
  ...
  // 4) Por defecto (revisión / limpieza / primera visita general): un GENERALISTA
  //    disponible (Irene o Mishelle). Se reparten entre ellas.
  const g = avail.find((p) => isGeneralist(p));
  if (g) return { professional: g, reason: "generalista" };

  // 5) No hay generalista con consulta a esa hora: NO asignamos a un especialista al
