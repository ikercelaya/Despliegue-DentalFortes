...
// Limpia el `content` que devuelve el modelo antes de reenviárselo: la API rechaza
// (400 invalid_request_error) los bloques de texto VACÍOS. Claude a veces emite un
// bloque {type:"text", text:""} justo antes de un tool_use (p. ej. al cancelar una cita,
// donde va directo a la herramienta sin texto previo); si lo reenviamos tal cual, falla.
function sanitizeAssistantContent(content) {
  if (!Array.isArray(content)) return content;
  const cleaned = content.filter((b) => {
    if (b && b.type === "text") return typeof b.text === "string" && b.text.trim().length > 0;
    return true; // conserva tool_use y cualquier otro bloque
  });
  // Nunca devolvemos un array vacío (la API también lo rechaza).
...
async function runAgent({ system, messages, tools = [] }) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    convo.push({ role: "assistant", content: sanitizeAssistantContent(resp.content) });
