...
// La conversación que se envía tiene que EMPEZAR y TERMINAR en el usuario. Si acaba en
// 'assistant', los modelos actuales lo interpretan como que se les está poniendo la
// respuesta en la boca y devuelven 400 ("does not support assistant message prefill").
// El historial ya llega ordenado desde lib/bot.js; esto es la última red por si alguna
// vez entra por otro sitio. Devolver [] es preferible a llamar a la API con basura:
// runAgent lo detecta y responde con el mensaje de cortesía en vez de reventar.
function normalizeEnds(msgs) {
...
async function runAgent({ system, messages, tools = [] }) {
  ...
  const convo = normalizeEnds(messages);
  if (!convo.length) {
    // Sin ningún mensaje del paciente al que contestar no hay nada que preguntarle al
    // modelo (y la API rechaza una conversación vacía).
    return { text: "", stop_reason: "sin_mensajes", messages: [] };
  }
